...
class _EmergencyContactsScreenState extends State<EmergencyContactsScreen>
              ...
              builder: (context, snapshot) {
                ...
                return Text(
                  count == 0
                      ...
                      : '$count contact${count == 1 ? '' : 's'} connected',
  ...
  Widget _buildTrustedCircleSection() => Container(
    ...
    child: Column(
      ...
      children: [
        const Row(
          children: [
            Icon(Icons.group_rounded, color: Color(0xFF9C27B0), size: 20),
            SizedBox(width: 8),
            Text(
              "Trusted Circle",
              style: TextStyle(
        ...
        SizedBox(
          ...
          child: StreamBuilder<List<EmergencyContact>>(
            stream: EmergencyContactService.getEmergencyContacts(),
            builder: (context, snapshot) {
              final contacts = (snapshot.data ?? []).take(8).toList();
              return ListView(
                scrollDirection: Axis.horizontal,
                children: [
                  ...contacts.map((c) => _buildTrustedAvatar(c.name)),
                  _buildAddTrustedButton(),
                ],
              );
            },
          ),
  ...
  Widget _buildTrustedAvatar(String label) {
    final initials = _initialsFromName(label);
    return Padding(
      ...
      child: Column(
        children: [
          CircleAvatar(
            radius: 22,
            backgroundColor: const Color(0xFFCE93D8).withOpacity(0.3),
            child: Text(
              initials,
              style: const TextStyle(
  ...
  Widget _buildAddTrustedButton() => Padding(
    padding: const EdgeInsets.only(right: 12),
    child: InkWell(
      ...
      child: Column(
        children: [
          CircleAvatar(
            ...
            child: const Icon(Icons.add, color: Color(0xFF00BCD4)),
          ),
          const SizedBox(height: 6),
          Text("Add", style: TextStyle(fontSize: 12, color: Colors.grey[700])),
        ],
      ),
    ),
  );
  ...
  // Widget _buildCheckInBanner() => Container(
  //   margin: const EdgeInsets.symmetric(horizontal: 20, vertical: 8),
    ...
    child: ListTile(
      ...
      subtitle: Column(
        ...
        children: [
          ...
          if (contact.locationLabel != null && contact.locationLabel!.isNotEmpty)
            Padding(
              padding: const EdgeInsets.only(top: 4),
              child: Row(
                children: [
                  const Icon(Icons.location_on_rounded, size: 16, color: Color(0xFF9C27B0)),
                  ...
                  content: SingleChildScrollView(
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          children: const [
                            Icon(Icons.info_outline, size: 16, color: Color(0xFF9C27B0)),
                            SizedBox(width: 6),
                            Expanded(
                              child: Text(
                                "Add location later by enabling geolocation.",
                                style: TextStyle(fontSize: 12, color: Color(0xFF7A1E6C)),
                              ),
                            ),
                          ],
                        ),
